...
echo -e "${CYAN}WISHPOOL — DEPLOY${NC}"
echo ""
...
WASM="target/wasm32-unknown-unknown/release/wishpool.wasm"
cd ..
...
echo -e "${YELLOW}[6/6] Making proof wish...${NC}"
TX_RESULT=$(stellar contract invoke \
  --network testnet --source deployer \
  --id ${CONTRACT_ID} \
  -- make_wish \
  --wisher ${DEPLOYER} \
  --text '"I wish for Stellar to reach 100 million users."' \
  --target 50000000 \
  --seed_amount 5000000 \
